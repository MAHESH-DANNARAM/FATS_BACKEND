...
def process_video_with_subtitles(youtube_url, MODEL_SIZE, TARGET_LANG_CODE, SUBTITLE_TYPE):
    # Load the Whisper ASR model
    ...
    output_video = OutputFile[:-4] + "_subtitled.mp4"
    os.system(f"ffmpeg -i {OutputFile} -vf subtitles={subtitles_} {output_video} -y")
